...
**首先**，需要注意的是`污水池模式`。它描述了这样一种场景，即一个请求经过多层传却没有执行任何逻辑操作。例如，展现层接受了用户查询客户数据的请求，然后传递给业务层，再传递到持久化层，持久层组装相应的查询SQL并传递到数据库层去获取数据。最后，从数据库查询到的数据没有经过任何处理（如整合、运算、数据转换）就一层一层地回传到了展现层。

每个分层架构的应用至少会有一些场景沦陷为`污水池模式`。但是，重要的还是分析应用中沦陷为`污水池模式`的请求的占比。`28原则`[^5]一直是一个用于检验你的项目是否遭遇了`污水池模式`的很好的实践。20%的请求只是在层与层之间简单传递，80%的请求不但在层与层传递，同时还有一些相应的逻辑处理。但是，如果你发现这个比率分配反过来了，即20%的请求在层间传递时有逻辑处理，80%的请求只是在层间传递，这时，你就应该考虑开放某些层了，这个工作比管理有隔离缺陷的层更难。
